...
class DemoteCommand extends Command {
    ...
    async run(ctx: CommandContext) {
        ...
        if(config.database.enabled) {
            ...
            .setDescription(`Are you sure that you want to demote **${ctx.args['roblox-user']}** to **${role.name}** with the reason \`${ctx.args['reason'] || 'No Reason Given'}\`?`)
            .setColor(mainColor);
            ...
            componentCollector.on('end', async collectedButtons => {
                if(collectedButtons.size === 0) {
                    if(ctx.subject instanceof CommandInteraction) {
                        ...
                        msg = await msg.edit({ embeds: [canceledEmbed] });
                    }

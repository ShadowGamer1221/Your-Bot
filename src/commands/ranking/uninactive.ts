...
class UninactiveCommand extends Command {
    constructor() {
        super({
            trigger: 'uninactive',
            ...
            args: [
                {
                    trigger: 'roblox-user',
                    description: 'Who do you want to uninactive?',
                    autocomplete: true,
                    type: 'String',
                },
                {
    ...
    async run(ctx: CommandContext) {
        ...
        if(!userData.inactiveUntil) return ctx.reply({ embeds: [ getNotInactiveEmbed() ] });

        const groupRoles = await robloxGroup.getRoles();
        const role = groupRoles.find((role) => role.id === userData.unactiveRank);
        if(!role) {
        ...
        await provider.updateUser(robloxUser.id.toString(), { inactiveUntil: null, unactiveRank: null });

        try {
            await robloxGroup.updateMember(robloxUser.id, role.id);
            ctx.reply({ embeds: [ await getSuccessfulUnactiveEmbed(robloxUser, role.name) ]});
            logAction('Uninactive', ctx.user, ctx.args['reason'], robloxUser, `${robloxMember.role.name} (${robloxMember.role.rank}) → ${role.name} (${role.rank})`);
        } catch (err) {
            console.error(err);
            return ctx.reply({ embeds: [ getUnexpectedErrorEmbed() ]});
        }
...
export default UninactiveCommand;